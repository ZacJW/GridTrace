...
config.read('/etc/gridsense/gridsense-data.conf')

config_auth = configparser.ConfigParser()
config_auth.read('/etc/gridsense/gridsense-data-auth.conf')

db_details = {"dbname": config.get('Database', 'name', fallback='gridsense'),
...
SSDP_SEARCH_TARGET = config.get('SSDP', 'search target', fallback='urn:grid-sense:service:data-probe:1')
